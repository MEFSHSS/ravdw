...
class WhoisEngine {
    ...
    async query(domain, options = {}) {
        ...
        try {
            ...
            const [rdapResult, whoisResult, hackerResult] = await Promise.allSettled(queryPromises);

            if (rdapResult.status === 'fulfilled') {
                results.sources.rdap = rdapResult.value;
            } else {
                results.errors.push(`RDAP: ${rdapResult.reason.message}`);
            }

            if (whoisResult.status === 'fulfilled') {
                results.sources.whois = whoisResult.value;
            } else {
                results.errors.push(`WHOIS: ${whoisResult.reason.message}`);
            }

            if (hackerResult.status === 'fulfilled') {
                results.sources.hackertarget = hackerResult.value;
            } else {
                results.errors.push(`HackerTarget: ${hackerResult.reason.message}`);
            }
    ...
    consolidateResults(...sources) {
        ...
        const validSources = sources.filter(s => s && !s.error);

        const allRegistrars = validSources.map(s => s.registrar).filter(Boolean);
        consolidated.registrar = this.mostCommonValue(allRegistrars);

        const registrarDetails = validSources
            .map(s => s.registrarDetails)
            .filter(Boolean)
            .find(details => details.ianaId) || validSources[0]?.registrarDetails;
    ...
    assessReliability(results) {
        const sources = Object.keys(results.sources).filter(key => 
            results.sources[key] && !results.sources[key].error
        );

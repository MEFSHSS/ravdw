...
function displayWhoisResults(result, lang) {
    const msg = {
        ...
        relatedDomains: i18n.t('lookup.related_domains', lang),
        reliabilityInfo: i18n.t('lookup.reliability_info', lang),
    ...
    if (result.whoisReliability) {
        const reliability = result.whoisReliability;
        const reliabilityLevel = reliability.level === 'HIGH' ? '🟢 ALTA' : 
                               reliability.level === 'MEDIUM' ? '🟡 MÉDIA' : '🔴 BAIXA';
        
        const reliabilityInfo = [
            chalk.white.bold(`Pontuação: `) + chalk.cyan(`${reliability.score}/100`),
            chalk.white.bold(`Nível: `) + chalk.yellow(reliabilityLevel),
            chalk.white.bold(`Fontes usadas: `) + chalk.green(reliability.sourcesUsed.join(', ')),
            chalk.white.bold(`Website: `) + chalk.blueBright(`https://secguide.pages.dev/gd/`),
            chalk.white.bold(`Contato de abuso: `) + (reliability.hasAbuseContact ? chalk.green('✓ Disponível') : chalk.red('✗ Não encontrado'))
        ];

        console.log(
            boxen(
                chalk.bold(msg.reliabilityInfo) + '\n' + reliabilityInfo.join('\n'),
                {
                    padding: 1,
                    borderColor: reliability.level === 'HIGH' ? 'green' : reliability.level === 'MEDIUM' ? 'yellow' : 'red',
                    borderStyle: 'classic',
                    margin: 1
                }
            )
    ...
    if (relevantContacts.length > 0) {
        ...
        for (const contact of relevantContacts) {
            ...
            const formatLongText = (text, maxWidth = 50) => {
                if (!text) return '';
            ...
            const contactLines = [
                ...
                '',
                `${contactIcon} ` + chalk.white('Tipo: ') + chalk.yellow(contactType),
                '📍 ' + chalk.white('Contato: ') + chalk.green(contact.contact || '—')
            ];

            if (contact.message) {
                const formattedMessage = formatLongText(contact.message);
                contactLines.push('', chalk.gray(formattedMessage));
            }

            if (contact.description) {
                const formattedDescription = formatLongText(contact.description);
                contactLines.push('', chalk.green(formattedDescription));
            }
    ...
    if (result.relatedDomains && result.relatedDomains.length > 0) {
        console.log(chalk.bold(`\n${msg.relatedDomains}`));
        
        const relatedBox = boxen(
            result.relatedDomains.map(domain => `• ${domain}`).join('\n'),
            {
                padding: 1,
                borderColor: 'blue',
                borderStyle: 'classic',
